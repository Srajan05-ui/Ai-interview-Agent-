...
function LoginForm() {
  ...
  const [loadingState, setLoadingState] = useState<'google' | 'github' | 'credentials' | null>(null);
        ...
        <div className="p-7 rounded-3xl bg-slate-900/70 border border-slate-800 shadow-2xl backdrop-blur-xl space-y-5">
          ...
          <div className="grid grid-cols-2 gap-3">
            <button
              ...
              className="flex items-center justify-center gap-2 py-2.5 px-3 rounded-xl bg-slate-950 hover:bg-slate-800/80 border border-slate-800 text-xs font-semibold text-slate-200 transition-all shadow-sm cursor-pointer disabled:opacity-50"
            >
              {loadingState === 'google' ? (
              ...
              {loadingState === 'google' ? 'Redirecting...' : 'Google'}
            </button>
            ...
            >
              ...
              {loadingState === 'github' ? 'Redirecting...' : 'GitHub'}
            </button>
          </div>
