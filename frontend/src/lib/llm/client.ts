...
import { StaticSignals } from '../github/analyzer';
...
Generate the next thoughtful, adaptive follow-up or probing question based on the candidate's last answer. Keep it natural, concise, and direct.`,
            },
          ],
        }),
      });
  ...
  // Realistic adaptive simulation based on turn & mode
  if (config.mode === 'Live Coding') {
    ...
    if (turnCount === 0) {
      ...
      return `That's interesting. When facing that roadblock with your team, how did you resolve disagreements regarding the architecture or timeline?`;
    } else {
...
export async function analyzeResumeContent(
  ...
  return {
    summary: `Resume strongly demonstrates backend and full-stack capabilities. Quantifiable achievements and metrics are prominent, though system-scale metrics and production impact could be highlighted further.`,
    sections: [
      {
        name: 'Experience & Impact',
        score: 85,
        feedback: 'Strong action verbs. Highlight user volume and throughput metrics (e.g., RPS, active users) where possible.',
      },
      {
        name: 'Technical Skills & Tools',
        score: 92,
        feedback: 'Modern and relevant stack keywords (TypeScript, Next.js, Docker, Cloud Services). Group by category for better ATS parsing.',
      },
      {
        name: 'Education & Certifications',
        score: 88,
        feedback: 'Clear credentials and timeline. Relevant coursework or honors can be added.',
      },
      {
        name: 'Projects & Portfolio',
        score: 82,
        feedback: 'Live demo and GitHub repository links are present. Clarify individual ownership vs. team contributions.',
      },
    ],
    bulletSuggestions: [
      {
...
export async function analyzeRepoWithLLM(
  ...
  // Compute realistic scores grounded in real static signals
  ...
  if (!signals.hasCi) {
    suggestions.push({
      ...
      description: 'Lack of automated CI workflow permits breaking regressions and lint errors to reach main branch.',
  ...
  // Analyze sample files for code quality
  ...
  if (sampleSource) {
    suggestions.push({
      ...
      suggestedFix: 'Wrap async I/O in structured try-catch blocks with customized application domain error types rather than catching generic errors.',
      codeSnippet: `try {\n  const res = await fetchData();\n} catch (err: unknown) {\n  logger.error('Failed fetching data', { err });\n  throw new ServiceUnavailableException('Upstream service error');\n}`,
    });
  }
